//! QuantumState - Manages the quantum properties of an entity, including its superposition and entanglement.
//! 
//! Generated from: QuantumState.schema.json
//! Category: components
//! Generated: 2025-06-20 02:54:12 UTC

use serde::{Deserialize, Serialize};

/// Manages the quantum properties of an entity, including its superposition and entanglement.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct QuantumState {
    /// Defines the common physics-related properties for components and laws.
    pub physics_properties: serde_json::Value,
    /// A map of field names to their definitions. Each field can be defined inline or by referencing a shared definition.
    pub fields: serde_json::Value,
}

impl QuantumState {
    /// Create a new QuantumState with default values
    pub fn new() -> Self {
        Self::default()
    }
}

impl Default for QuantumState {
    fn default() -> Self {
        Self {
            physics_properties: serde_json::Value::Null,
            fields: serde_json::Value::Null,
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_quantum_state_creation() {
        let instance = QuantumState::new();
        assert_eq!(instance, QuantumState::default());
    }

    #[test]
    fn test_quantum_state_serialization() {
        let instance = QuantumState::default();
        
        // Test JSON serialization
        let json = serde_json::to_string(&instance).expect("Failed to serialize");
        let deserialized: QuantumState = serde_json::from_str(&json).expect("Failed to deserialize");
        
        assert_eq!(instance, deserialized);
    }
}